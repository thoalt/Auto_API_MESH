...
class networkinfoViewClient(BaseClient):
    def __init__(self):
        super().__init__()
        self.url = cfg.url_Agent
        self.request = API_lib()

    def Create_networkinfoView_Pload(self, action=None, reqID=None):
        pload = cfg.req_networkinfoView
        payload = self.set_payload_with_action_reqID(pload=pload, action=action, reqID=reqID)
        return payload

    def networkinfoView(self, cookies=None, pload=None):
        if pload is None:
            payload = self.Create_networkinfoView_Pload()
        else:
            payload = pload
