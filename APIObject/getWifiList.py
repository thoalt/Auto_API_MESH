...
class getWifiListClient(BaseClient):
    def __init__(self):
        super().__init__()
        self.url = cfg.url_Agent
        self.request = API_lib()

    def Create_getWifiList_Pload(self, action=None, reqID=None):
        pload = cfg.req_getWifiList
        payload = self.set_payload_with_action_reqID(pload=pload, action=action, reqID=reqID)
        return payload

    def getWifiList(self, cookies=None, pload=None):
        if pload is None:
            payload = self.Create_getWifiList_Pload()
        else:
            payload = pload
