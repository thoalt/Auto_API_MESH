...
class Test_radio5G_Edit():
    ...
    def set_up(self):
        ...
        self.bandwith_API_lst = [BAND_WIDTH_API.B_80]
        self.bandwith_GUI_exp = [BAND_WITH_GUI.B_80]
    ...
    def test_RADIO5G_EDIT_BW_1(self):
        ...
        for item in self.bandwith_API_lst:
            ...
            standard = self.wrp.get_Standard5G_sp()
            self.radio5GViewClt.assert_val(STANDARD_5G.MOD_AC, standard)

            # Getting Channel from driver
            bitrate = self.session.get_bitrate(cfg.WIFI_INT_5G)
            bandW = self.client5G.Convert_Bitrate_To_Bandwith(standard=STANDARD_5G.MOD_AC, bitrate=bitrate)
